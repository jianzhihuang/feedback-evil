...
```bash
# 版本查詢
uvx feedback-evil@latest version       # 檢查版本

# 介面測試
uvx feedback-evil@latest test --web    # 測試 Web UI (自動持續運行)
uvx feedback-evil@latest test --desktop # 測試桌面應用程式 (v2.5.0 新功能)

# 調試模式
MCP_DEBUG=true uvx feedback-evil@latest test

# 指定語言測試
MCP_LANGUAGE=en uvx feedback-evil@latest test --web    # 強制使用英文介面
MCP_LANGUAGE=zh-TW uvx feedback-evil@latest test --web  # 強制使用繁體中文
MCP_LANGUAGE=zh-CN uvx feedback-evil@latest test --web  # 強制使用簡體中文
```
...
uvx --no-cache --with-editable . feedback-evil test --web   # Web UI 測試 (持續運行)
uvx --no-cache --with-editable . feedback-evil test --desktop # 桌面應用測試
...
**Q: 中文字符亂碼**
A: 已在 v2.0.3 修復。更新到最新版本：`uvx feedback-evil@latest`
